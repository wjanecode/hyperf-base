...
$wgwSecretList = env('WGW_SECRET_LIST'); // 密钥对列表 test&abcdefg;test1&abcdefg
$appSecretList = [];
if (isset($wgwSecretList)) {
    $wgwSecretList = explode(';', $wgwSecretList);
    array_map(function (string $item) use (&$appSecretList) {
        if (Str::contains($item, '&')) {
            ...
            if (count($itemArray) == 2) {
                ...
                Log::error('wgw secret item invalidate & explode count over 2!');
            }
        } else {
            Log::error('wgw secret item invalidate & not found!');
        }
    }, $wgwSecretList);
}
...
return [
    'wgw' => [
        'force_auth' => env('WGW_FORCE_AUTH', false), // 强制校验签名,开启后WGW协议必须带签名参数访问
        'sign_ttl' => env('WGW_SIGN_TTL', 10), // 签名有效时间
